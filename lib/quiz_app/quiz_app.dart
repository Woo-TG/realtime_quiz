...
import 'package:realtime_quiz/model/quiz.dart';
...
class _QuizPageState extends State<QuizPage> {
  ...
  Future calcResult() async {
    if (!isDone) {
      ...
      var scoredkeys = countMap.keys.toList(growable: false)
        ..sort((k1, k2) => countMap[k2]!.compareTo(countMap[k1]!));

      setState(() {
        quizResult = scoredkeys;
      });
    }
  }
                      ...
                      if (currentIndex < problemsSets.length) {
                        ...
                        return Container(
                          ...
                          child: _ProblemSolveWidget(
                            ...
                            startTime: problemTriggers[currentIndex]["start"] ?? 0,
...
class _ProblemSolveWidgetState extends State<_ProblemSolveWidget> {
  ...
  Future periodicTask() async {
    final startTime = DateTime.fromMicrosecondsSinceEpoch(widget.startTime);
    final endTime = DateTime.fromMicrosecondsSinceEpoch(widget.endTime);

    timer ??= Timer.periodic(const Duration(seconds: 1), (t) {
      DateTime nowDatatime = DateTime.now();
      final sDiff = nowDatatime.difference(startTime);
      final eDiff = endTime.difference(nowDatatime);
